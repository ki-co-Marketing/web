...
<div class="initiative-single-header" style="background:var(--color-primary);color:#fff;padding:var(--space-12) 0">
	<div class="container">
		...
		<div style="display:flex;gap:var(--space-3);flex-wrap:wrap;margin-bottom:var(--space-4)">
			<?php echo urban_cms_initiative_status_badge( $post_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- trusted theme function ?>
		...
		<div class="initiative-metrics" style="display:flex;gap:var(--space-8);flex-wrap:wrap;margin-top:var(--space-8);padding-top:var(--space-6);border-top:1px solid rgba(255,255,255,0.15)">
			...
			<div>
				...
				<div style="font-weight:700"><?php echo esc_html( gmdate( 'F Y', strtotime( $start ) ) ); ?></div>
			</div>
			<?php endif; ?>
			<?php if ( $end ) : ?>
			<div>
				...
				<div style="font-weight:700"><?php echo esc_html( gmdate( 'F Y', strtotime( $end ) ) ); ?></div>
			</div>
			<?php endif; ?>
			<?php if ( $budget ) : ?>
			<div>
			...
			<div class="widget">
				...
				<dl style="font-size:var(--text-sm);display:flex;flex-direction:column;gap:var(--space-3)">
					<div>
						...
						<dd><?php echo urban_cms_initiative_status_badge( $post_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- trusted theme function ?></dd>
					</div>
					<?php if ( $progress > 0 ) : ?>
					<div>
