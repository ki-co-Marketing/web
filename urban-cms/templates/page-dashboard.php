...
<div class="container" style="padding-top:var(--space-10);padding-bottom:var(--space-16)">
    ...
    <div style="display:grid;grid-template-columns:220px 1fr;gap:var(--space-10);align-items:start">
        ...
        <nav class="dashboard-nav" aria-label="<?php esc_attr_e( 'Dashboard navigation', 'urban-cms' ); ?>">
            <?php
            $current_section = sanitize_text_field( $_GET['section'] ?? 'overview' );
            $sections = [
...
function urban_cms_dashboard_overview( WP_User $user ): void {
    ...
    $biz_count   = ( new WP_Query( array_merge( $biz_args, [ 'post_status' => 'publish', 'posts_per_page' => -1 ] ) ) )->found_posts;
    $ev_count    = ( new WP_Query( array_merge( $ev_args,  [ 'post_status' => 'publish', 'posts_per_page' => -1 ] ) ) )->found_posts;
    $draft_count = ( new WP_Query( [ 'post_type' => [ 'urban_business', 'urban_event' ], 'post_status' => 'draft', 'author' => $user->ID, 'posts_per_page' => -1 ] ) )->found_posts;
    ?>
...
function urban_cms_dashboard_businesses( WP_User $user ): void {
    ...
    <?php urban_cms_render_submission_table( $user->ID, 'urban_business' );
}
...
function urban_cms_dashboard_events( WP_User $user ): void {
    ...
    <?php urban_cms_render_submission_table( $user->ID, 'urban_event' );
}
...
function urban_cms_render_submission_table( int $user_id, string $post_type ): void {
    ...
    $meta_email = get_user_meta( $user_id, '_submission_email', true ) ?: get_userdata( $user_id )->user_email;
    $args_meta  = array_merge( $args, [
        'author'     => 0,
        'meta_query' => [ [ 'key' => '_submission_email', 'value' => $meta_email, 'compare' => '=' ] ],
    ] );
    ...
    foreach ( $posts as $post ) {
        ...
        $status_map = [
            'publish' => [ __( 'Published', 'urban-cms' ), 'badge--success' ],
            'draft'   => [ __( 'Pending Review', 'urban-cms' ), 'badge' ],
            'pending' => [ __( 'Pending Review', 'urban-cms' ), 'badge' ],
        ];
        [ $status_label, $badge_class ] = $status_map[ $status ] ?? [ ucfirst( $status ), 'badge' ];
