...
function urban_cms_event_time( int $post_id ): string {
	...
	$fmt_start = gmdate( 'g:i a', strtotime( $start ) );
	if ( $end ) {
		$fmt_end = gmdate( 'g:i a', strtotime( $end ) );
...
function urban_cms_map_data(): array {
	...
	foreach ( $businesses as $b ) {
		...
		$markers[] = array(
			...
			'lat'      => $lat,
			'lng'      => $lng,
			'logo'     => get_the_post_thumbnail_url( $b->ID, 'urban-thumb' ) ? get_the_post_thumbnail_url( $b->ID, 'urban-thumb' ) : '',
			'category' => implode( ', ', wp_get_post_terms( $b->ID, 'business_category', array( 'fields' => 'names' ) ) ),
		);
	}
...
function urban_cms_initiative_status_badge( int $post_id ): string {
	$raw_status        = get_post_meta( $post_id, '_initiative_status', true );
	$status            = $raw_status ? $raw_status : 'active';
	$map               = array(
		'planning'  => array( 'Planning', 'badge--primary' ),
		'active'    => array( 'Active', 'badge--success' ),
		'on-hold'   => array( 'On Hold', 'badge' ),
		'completed' => array( 'Completed', 'badge--accent' ),
	);
	[ $label, $class ] = $map[ $status ] ?? array( ucfirst( $status ), 'badge' );
...
function urban_cms_pagination( WP_Query $query = null ): void {
	...
	foreach ( $links as $link ) {
		echo '<li class="pagination__item">' . $link . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- paginate_links() output is trusted WP HTML
	}
	echo '</ul></nav>';
}
...
function urban_cms_section_header( string $title, string $subtitle = '', string $view_all_url = '', string $view_all_text = '' ): void {
	...
	if ( $subtitle ) {
		echo '<p class="color-muted">' . esc_html( $subtitle ) . '</p>';
	}
	echo '</div>';
	if ( $view_all_url ) {
		echo '<a href="' . esc_url( $view_all_url ) . '" class="btn btn--outline btn--sm">'
			. esc_html( $view_all_text ? $view_all_text : __( 'View All', 'urban-cms' ) )
			. '</a>';
	}
	echo '</div>';
}
