...
function urban_cms_register_events() {
    ...
    register_post_type( 'urban_event', [
        'labels'            => $labels,
        'public'            => true,
        'has_archive'       => 'events',
...
function urban_cms_save_event_meta( int $post_id, WP_Post $post ) {
    if ( ! isset( $_POST['urban_event_nonce'] ) ) return;
    if ( ! wp_verify_nonce( $_POST['urban_event_nonce'], 'urban_event_meta' ) ) return;
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
    if ( ! current_user_can( 'edit_post', $post_id ) ) return;

    $fields = [
        '_event_start_date' => [ 'event_start_date', 'sanitize_text_field' ],
        '_event_end_date'   => [ 'event_end_date',   'sanitize_text_field' ],
        '_event_start_time' => [ 'event_start_time', 'sanitize_text_field' ],
        '_event_end_time'   => [ 'event_end_time',   'sanitize_text_field' ],
        '_event_location'   => [ 'event_location',   'sanitize_text_field' ],
        '_event_address'    => [ 'event_address',    'sanitize_text_field' ],
        '_event_cost'       => [ 'event_cost',       'sanitize_text_field' ],
        '_event_rsvp_url'   => [ 'event_rsvp_url',   'esc_url_raw' ],
        '_event_organizer'  => [ 'event_organizer',  'sanitize_text_field' ],
    ];
...
function urban_cms_register_businesses() {
    ...
    register_post_type( 'urban_business', [
        'labels'        => $labels,
        'public'        => true,
        ...
        'taxonomies'    => [ 'business_category', 'district_neighborhood' ],
    ] );
...
function urban_cms_business_meta_box( WP_Post $post ) {
    ...
    $fields = [
        '_business_address'   => [ 'Address',           'text',  '' ],
        '_business_suite'     => [ 'Suite / Floor',     'text',  '' ],
        '_business_phone'     => [ 'Phone',             'tel',   '' ],
        '_business_email'     => [ 'Email',             'email', '' ],
        '_business_website'   => [ 'Website URL',       'url',   '' ],
        '_business_hours'     => [ 'Hours',             'text',  'e.g. Mon-Fri 9am–6pm' ],
        '_business_instagram' => [ 'Instagram URL',     'url',   '' ],
        '_business_facebook'  => [ 'Facebook URL',      'url',   '' ],
        '_business_lat'       => [ 'Latitude',          'text',  'e.g. 47.6062' ],
        '_business_lng'       => [ 'Longitude',         'text',  'e.g. -122.3321' ],
        '_business_featured'  => [ 'Featured Listing',  'checkbox', '' ],
    ];
...
function urban_cms_save_business_meta( int $post_id, WP_Post $post ) {
    if ( ! isset( $_POST['urban_business_nonce'] ) ) return;
    if ( ! wp_verify_nonce( $_POST['urban_business_nonce'], 'urban_business_meta' ) ) return;
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
    if ( ! current_user_can( 'edit_post', $post_id ) ) return;
...
function urban_cms_register_initiatives() {
    ...
    register_post_type( 'urban_initiative', [
        'labels'        => $labels,
        'public'        => true,
...
function urban_cms_initiative_meta_box( WP_Post $post ) {
    ...
    $status   = get_post_meta( $post->ID, '_initiative_status',   true ) ?: 'active';
    $progress = get_post_meta( $post->ID, '_initiative_progress', true ) ?: 0;
...
function urban_cms_save_initiative_meta( int $post_id, WP_Post $post ) {
    if ( ! isset( $_POST['urban_initiative_nonce'] ) ) return;
    if ( ! wp_verify_nonce( $_POST['urban_initiative_nonce'], 'urban_initiative_meta' ) ) return;
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
    if ( ! current_user_can( 'edit_post', $post_id ) ) return;

    $allowed_statuses = [ 'planning', 'active', 'on-hold', 'completed' ];
    $status = sanitize_text_field( $_POST['initiative_status'] ?? 'active' );
    ...
    update_post_meta( $post_id, '_initiative_budget',   sanitize_text_field( $_POST['initiative_budget']   ?? '' ) );
    update_post_meta( $post_id, '_initiative_start',    sanitize_text_field( $_POST['initiative_start']    ?? '' ) );
    update_post_meta( $post_id, '_initiative_end',      sanitize_text_field( $_POST['initiative_end']      ?? '' ) );
    update_post_meta( $post_id, '_initiative_lead',     sanitize_text_field( $_POST['initiative_lead']     ?? '' ) );
    update_post_meta( $post_id, '_initiative_location', sanitize_text_field( $_POST['initiative_location'] ?? '' ) );
}
...
function urban_cms_team_meta_box( WP_Post $post ) {
    ...
    $fields = [
        '_team_title'    => [ 'Job Title / Role',  'text', '' ],
        '_team_email'    => [ 'Email',             'email', '' ],
        '_team_phone'    => [ 'Phone',             'tel',   '' ],
        '_team_linkedin' => [ 'LinkedIn URL',      'url',   '' ],
    ];
...
function urban_cms_save_team_meta( int $post_id, WP_Post $post ) {
    if ( ! isset( $_POST['urban_team_nonce'] ) ) return;
    if ( ! wp_verify_nonce( $_POST['urban_team_nonce'], 'urban_team_meta' ) ) return;
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
    if ( ! current_user_can( 'edit_post', $post_id ) ) return;
