...
function urban_cms_handle_newsletter_subscribe(): never {
	...
	if ( (int) get_transient( $rl_key ) >= 5 ) {
		wp_send_json_error(
			array(
				'code'    => 'rate_limit',
				'message' => __( 'Too many requests. Please try again later.', 'urban-cms' ),
			),
			429
		);
	}
	...
	if ( ! $email || ! is_email( $email ) ) {
		wp_send_json_error(
			array(
				'code'    => 'invalid_email',
				'message' => __( 'Please enter a valid email address.', 'urban-cms' ),
			),
			422
		);
	}
	...
	$lists      = sanitize_text_field( wp_unslash( $_POST['lists'] ?? 'general' ) );
	...
	$existing = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE email = %s", $email ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared
...
add_action(
	'init',
	function () {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- tokenized email confirmation link, no nonce needed
		if ( isset( $_GET['urban_confirm_sub'] ) ) {
			$token = sanitize_text_field( wp_unslash( $_GET['urban_confirm_sub'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( $token ) {
...
function urban_cms_confirm_subscription( string $token ): void {
	...
	$sub = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE token = %s AND status = 'pending'", $token ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared
...
function urban_cms_handle_newsletter_confirm(): never {
	...
	wp_send_json_error( array( 'code' => 'use_link' ), 400 );
}
...
function urban_cms_admin_subscribers_page(): void {
	...
	// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- absint-cast integers are safe
	echo '<p>' . sprintf( esc_html__( '%1$d active, %2$d pending confirmation.', 'urban-cms' ), absint( $active ), absint( $pending ) ) . '</p>';
	...
	if ( ! empty( $_GET['export'] ) && current_user_can( 'manage_options' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- admin-only export, capability check is sufficient
	...
	foreach ( (array) $rows as $row ) {
		...
		echo '<td>' . esc_html( $row->subscribed ? $row->subscribed : '—' ) . '</td>';
		echo '</tr>';
	}
...
function urban_cms_export_subscribers_csv( array $rows ): void {
	...
	// phpcs:disable WordPress.WP.AlternativeFunctions.file_system_operations_fopen,WordPress.WP.AlternativeFunctions.file_system_operations_fclose -- php://output stream, WP_Filesystem does not support it
	$out = fopen( 'php://output', 'w' ); // phpcs:ignore
	fputcsv( $out, array( 'Email', 'First Name', 'Last Name', 'Status', 'Lists', 'Source', 'Subscribed', 'Created' ) );
	foreach ( $rows as $row ) {
		fputcsv( $out, array( $row->email, $row->first_name, $row->last_name, $row->status, $row->lists, $row->source, $row->subscribed, $row->created_at ) );
	}
