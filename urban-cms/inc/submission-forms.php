...
add_action( 'wp_enqueue_scripts', function () {
    if ( ! is_page_template( [ 'templates/page-submit-business.php', 'templates/page-submit-event.php' ] ) ) {
        return;
    }
    wp_enqueue_script(
        'urban-cms-forms',
    ...
    wp_localize_script( 'urban-cms-forms', 'urbanForms', [
...
function urban_cms_handle_business_submission(): never {
    ...
    if ( ! empty( $_POST['website_url_hp'] ) ) {
        wp_send_json_success( [ 'honeypot' => true ] ); // Silently succeed to fool bots
    }

    // 3. Rate limit
    if ( ! urban_cms_check_rate_limit( 'biz' ) ) {
        wp_send_json_error( [ 'code' => 'rate_limit', 'message' => __( 'Rate limit exceeded. Please wait before submitting again.', 'urban-cms' ) ], 429 );
    }

    // 4. Validate required fields
    $name = sanitize_text_field( $_POST['business_name'] ?? '' );
    if ( ! $name ) {
        wp_send_json_error( [ 'code' => 'missing_name', 'message' => __( 'Business name is required.', 'urban-cms' ) ], 422 );
    }

    $submitter_email = sanitize_email( $_POST['submitter_email'] ?? '' );
    if ( ! $submitter_email || ! is_email( $submitter_email ) ) {
        wp_send_json_error( [ 'code' => 'invalid_email', 'message' => __( 'A valid contact email is required.', 'urban-cms' ) ], 422 );
    }
    ...
    if ( is_wp_error( $post_id ) ) {
        wp_send_json_error( [ 'code' => 'insert_failed', 'message' => $post_id->get_error_message() ], 500 );
    }

    // 6. Save meta
    $meta_map = [
        '_business_address'   => sanitize_text_field( $_POST['address']   ?? '' ),
        '_business_suite'     => sanitize_text_field( $_POST['suite']     ?? '' ),
        '_business_phone'     => sanitize_text_field( $_POST['phone']     ?? '' ),
        '_business_email'     => sanitize_email( $_POST['business_email'] ?? '' ),
        '_business_website'   => esc_url_raw( $_POST['website']           ?? '' ),
        '_business_hours'     => sanitize_text_field( $_POST['hours']     ?? '' ),
        '_business_instagram' => esc_url_raw( $_POST['instagram']         ?? '' ),
        '_business_facebook'  => esc_url_raw( $_POST['facebook']          ?? '' ),
        '_business_lat'       => sanitize_text_field( $_POST['lat']       ?? '' ),
        '_business_lng'       => sanitize_text_field( $_POST['lng']       ?? '' ),
    ...
    if ( $category_slug ) {
        ...
        if ( $term ) wp_set_post_terms( $post_id, [ $term->term_id ], 'business_category' );
    }
    ...
    if ( is_wp_error( $attachment_id ) ) {
        ...
        wp_send_json_error( [ 'code' => 'upload_failed', 'message' => $attachment_id->get_error_message() ], 422 );
    }
    if ( $attachment_id ) {
...
function urban_cms_handle_event_submission(): never {
    ...
    if ( ! $title ) {
        wp_send_json_error( [ 'code' => 'missing_title', 'message' => __( 'Event title is required.', 'urban-cms' ) ], 422 );
    }

    $start_date = sanitize_text_field( $_POST['start_date'] ?? '' );
    if ( ! $start_date || ! strtotime( $start_date ) ) {
        wp_send_json_error( [ 'code' => 'missing_date', 'message' => __( 'A valid start date is required.', 'urban-cms' ) ], 422 );
    }
    ...
    $meta_map = [
        ...
        '_event_location'   => sanitize_text_field( $_POST['location']  ?? '' ),
        '_event_address'    => sanitize_text_field( $_POST['address']   ?? '' ),
        '_event_cost'       => sanitize_text_field( $_POST['cost']      ?? '' ),
        '_event_rsvp_url'   => esc_url_raw( $_POST['rsvp_url']          ?? '' ),
        '_event_organizer'  => sanitize_text_field( $_POST['organizer'] ?? '' ),
        '_submission_ip'    => urban_cms_get_ip(),
        '_submission_email' => $submitter_email,
        '_submission_date'  => current_time( 'mysql' ),
    ];
    ...
    if ( $category_slug ) {
        ...
        if ( $term ) wp_set_post_terms( $post_id, [ $term->term_id ], 'event_category' );
    }
