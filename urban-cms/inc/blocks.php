...
function urban_cms_render_initiative_progress( array $attrs ): string {
	...
	echo '<div class="initiative-card__status">' . urban_cms_initiative_status_badge( $post_id ) . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- output of trusted theme function
	...
	if ( $show_det ) {
		...
		if ( $lead ) {
			echo '<div><dt style="font-weight:700;color:var(--color-primary)">' . esc_html__( 'Lead', 'urban-cms' ) . '</dt><dd>' . esc_html( $lead ) . '</dd></div>';
		}
		if ( $budget ) {
			echo '<div><dt style="font-weight:700;color:var(--color-primary)">' . esc_html__( 'Budget', 'urban-cms' ) . '</dt><dd>' . esc_html( $budget ) . '</dd></div>';
		}
		if ( $end ) {
			echo '<div><dt style="font-weight:700;color:var(--color-primary)">' . esc_html__( 'Target', 'urban-cms' ) . '</dt><dd>' . esc_html( gmdate( 'F Y', strtotime( $end ) ) ) . '</dd></div>';
		}
		echo '</dl>';
	}
...
function urban_cms_render_newsletter( array $attrs ): string {
	...
	if ( $heading ) {
		add_filter( 'theme_mod_newsletter_heading', fn() => $heading );
	}
	if ( $subheading ) {
		add_filter( 'theme_mod_newsletter_subheading', fn() => $subheading );
	}
	...
	if ( $heading ) {
		remove_all_filters( 'theme_mod_newsletter_heading' );
	}
	if ( $subheading ) {
		remove_all_filters( 'theme_mod_newsletter_subheading' );
	}
...
function urban_cms_render_hero_banner( array $attrs ): string {
	...
	$bg_color_raw = sanitize_hex_color( $attrs['bgColor'] ?? '#1a3a5c' );
	$bg_color     = $bg_color_raw ? $bg_color_raw : '#1a3a5c';
	$accent_raw   = sanitize_hex_color( $attrs['accentColor'] ?? '#e8a020' );
	$accent       = $accent_raw ? $accent_raw : '#e8a020';
